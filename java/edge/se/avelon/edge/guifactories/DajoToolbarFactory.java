...
import se.avelon.edge.application.MyMaxNuberOfRows;
...
public class DajoToolbarFactory {

	public static JToolBar getJToolBar(String file) {
		try {
			Document doc =  DajoXmlUtil.getDocument(file);
			return (JToolBar)DajoToolbarFactory.getComponent(doc.getChildNodes().item(0));
		} 
	...
	private static JComponent getComponent(Node node) {
		...
		for(int i = 0 ; i < nodes.getLength(); i++) {
			if(comp != null  && nodes.item(i).getNodeType() == Node.ELEMENT_NODE) {
				JComponent c = DajoToolbarFactory.getComponent(nodes.item(i));
				if(c == null) {
