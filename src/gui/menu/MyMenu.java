...
public class MyMenu {
	...
	public static JMenuBar getJMenuBar() {
		try {
			Document doc =  MyXmlUtil.getDocument("etc/menu.xml");
			return (JMenuBar)MyMenu.getComponent(doc.getChildNodes().item(0));
		} 
	...
	private static JComponent getComponent(Node node) {
		...
		for(int i = 0 ; i < nodes.getLength(); i++) {
			if(comp != null  && nodes.item(i).getNodeType() == Node.ELEMENT_NODE) {
				JComponent c = MyMenu.getComponent(nodes.item(i));
				if(c == null) {
